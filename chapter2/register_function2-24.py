...
class RegisterFunction:
    def __init__(self,url,order):
        '''构造函数'''
        self.driver = self.initDriver(url,order)

    def initDriver(self,url,order):
        if order == 0:
            '''初始化driver'''
    ...
    def main(self):
        try:
            ...
            '''验证码输入111111必定是错误的---------人为制造的错误'''
            self.sendInputVal("code","111111")
            time.sleep(2)
            self.getElement("register_button").click()
            codeErrorElement = self.getElement("code_error")
            if codeErrorElement:
                '''***********保存error照片的时候用随机数************'''
                random = self.getRandomValue()
                errImgName = "code_error" + random + ".png"
                errorPath = "D:/D1/code/AutoTest/python_ui_autotest/SeleniumPython/chapter2/image/" + errImgName
                self.saveScreenshot(errorPath)
            else:
                print("注册成功")
            time.sleep(5)
            ...
            try:
                ...
                print("msedgedriver driver进程不存在")

if(__name__ == "__main__"):
    url = "http://www.5itest.cn/register"
    for item in range(3):
        registerFunction = RegisterFunction(url,item)
